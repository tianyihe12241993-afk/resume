...
class CalendarEvent(Base):
    """Interview / reminder events shown on the calendar. Owned by a user;
    optionally linked to a job (job_url_id) and/or profiles (profile_ids_json)."""
    ...
    # Optional link to a specific tailored job row (nullable; no hard FK so a
    # deleted job doesn't cascade away the calendar event).
    job_url_id: Mapped[Optional[int]] = mapped_column(Integer, nullable=True)
    # JSON list of profile ids this event relates to.
    profile_ids_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
