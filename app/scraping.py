...
def list_ats_postings(provider: str, slug: str) -> list[dict]:
    """List ALL open roles for one ATS company board.

    `provider` is greenhouse|lever|ashby; `slug` is the company's board id
    (e.g. "stripe"). Returns a list of
    {url, company, title, location, work_type, description, source} dicts.
    ...
    try:
        if provider == "greenhouse":
            rows = _list_greenhouse(slug)
        elif provider == "lever":
            rows = _list_lever(slug)
        elif provider == "ashby":
            rows = _list_ashby(slug)
        else:
            return []
    except Exception:
        return []
    return [r for r in (_finalize(r) for r in rows) if r and r.get("url")]


def _list_greenhouse(slug: str) -> list[dict]:
...
def _list_lever(slug: str) -> list[dict]:
    ...
    for d in r.json():
        ...
        out.append({
            ...
            "work_type": d.get("workplaceType") or cats.get("workplaceType"),
            "description": _html_to_text(desc_html),
            "source": "ats:lever",
        })
    return out
...
def _list_ashby(slug: str) -> list[dict]:
    ...
    for p in data.get("jobs", []):
        url = p.get("jobUrl") or p.get("applyUrl") or (
            f"https://jobs.ashbyhq.com/{slug}/{p.get('id')}" if p.get("id") else ""
        )
        out.append({
            "url": url,
            "company": company,
            ...
            "description": (
                p.get("descriptionPlain") or _html_to_text(p.get("descriptionHtml", ""))
            ),
            "source": "ats:ashby",
        })
    return out
